...
def create_spark_session() -> SparkSession:
    ...
    return (
        ...
        .appName("InsuranceDataTransformation")
...
def validate_data(df: DataFrame) -> DataFrame:
    ...
    df = df.filter(
        ...
        (F.col("bmi") > 0) & (F.col("charges") > 0)
    )

    final_count = df.count()
    dropped = initial_count - final_count
    logger.info(f"Validation: {initial_count} in → "
                f"{final_count} valid →  {dropped} dropped"
                )
    return df
...
def transform_data(df: DataFrame) -> DataFrame:
    ...
    df = df.withColumn(
        "ingestion_timestamp", 
        F.lit(datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S")))
